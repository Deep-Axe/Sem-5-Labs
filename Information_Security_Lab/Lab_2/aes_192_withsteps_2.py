...
def key_to_words(key_bytes):
    words = []
    for i in range(0, len(key_bytes), 4):
        words.append(int.from_bytes(key_bytes[i:i+4], 'big'))
    return words
...
def message_to_state(msg):
    return np.array(list(msg), dtype=np.uint8).reshape(4,4, order='F')

# AddRoundKey
def add_round_key(state, round_key_words):
    rk_bytes = b''.join([w.to_bytes(4,'big') for w in round_key_words])
    rk_state = np.array(list(rk_bytes), dtype=np.uint8).reshape(4,4, order='F')
    return state ^ rk_state
...
def aes_encrypt(message_bytes, key_bytes):
    state = message_to_state(message_bytes)
    words = expand_key(key_bytes)
    Nr = 12
    ...
    # Initial Round
    state = add_round_key(state, words[:4])
    ...
    for r in range(1, Nr):
        ...
        state = add_round_key(state, words[4*r:4*(r+1)])
        print(f"\nAfter Round {r}:")
        print(state)
    ...
    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, words[4*Nr:4*(Nr+1)])
    print("\nAfter Final Round:")
    print(state)
    
    return state

# Run AES-192 encryption

"""Hybrid Rabin + AES-128 demo (ECB, CBC, CFB, OFB, CTR).

This file implements a minimal Rabin cryptosystem (keygen/encrypt/decrypt)
for educational key-wrapping. The Rabin decryption returns four roots; we
embed a short marker in the plaintext so the correct root can be selected.
"""
...
def wrap_key(priv_pub: dict, key_bytes: bytes) -> bytes:
    # public component is n
    n = priv_pub['n']
...
def unwrap_key(priv: dict, wrapped: bytes, expected_len: int) -> bytes:
    ...
    for r in roots:
        ...
        # allow for leading zeros if expected_len small

"""Hybrid ElGamal + AES-256 demo (ECB, CBC, CFB, OFB, CTR)."""
...
def aes_demo(key_bytes: bytes, plaintext: bytes):
    print('\n=== AES-256 Demo ===')
    print_hex('AES-256 key', key_bytes)
...
def main():
    ...
    print_hex('\nGenerated AES-256 key', key_bytes)

    wrapped = elgamal_wrap(pub, key_bytes)
    print_hex('\nWrapped AES key (ElGamal)', wrapped)

    unwrapped = elgamal_unwrap(priv, wrapped, expected_len=32)
